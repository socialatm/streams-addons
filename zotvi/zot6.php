...
//		$ret['signed_token'] = base64url_encode(rsa_sign('token.' . $token,$e['channel_prvkey'],$sig_method));
	...
	$ret['guid_sig']       = \Zotlabs\Lib\JSalmon::sign($e['xchan_guid'],z_root() . '/channel/' . $e['channel_address'], $e['channel_prvkey']);
	...
	if($x && count($x)) {
		foreach($x as $hub) {
			...
			$ret[] = [
				...
				'url'      => $hub['hubloc_url'],
				'url_sig'  => \Zotlabs\Lib\JSalmon::sign($hub['hubloc_url'],z_root() . '/channel/' . $channel['channel_address'],$channel['channel_prvkey']),
...
static function zot_site_info() {
	...
	$ret['site'] = [];
	$ret['site']['url'] = z_root();
	$ret['site']['url_sig'] = \Zotlabs\Lib\JSalmon::sign(z_root(), z_root(), $signing_key);
