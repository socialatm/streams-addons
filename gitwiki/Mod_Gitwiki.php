...
class Gitwiki extends \Zotlabs\Web\Controller {
	...
	function get() {
		...
		// init() should have forced the URL to redirect to /gitwiki/channel so assume argc() > 1
		$nick = argv(1);
		...
		switch (argc()) {
			case 2:
				...
				if ($wikis) {
					$o .= replace_macros(get_markup_template('gitwikilist.tpl','addon/gitwiki'), array(
						'$header' => t('Wikis'),
				...
				break;
			case 3:
				// /gitwiki/channel/wiki -> No page was specified, so redirect to Home.md
				$wikiUrlName = urlencode(argv(2));
				goaway('/'.argv(0).'/'.argv(1).'/'.$wikiUrlName.'/Home');
			case 4:
				// GET /gitwiki/channel/wiki/page
		...
		$o .= replace_macros(get_markup_template('gitwiki.tpl','addon/gitwiki'),array(
	...
	function post() {
		...
		if ((argc() > 3) && (argv(2) === 'create') && (argv(3) === 'wiki')) {
			...
			if ($r['success']) {
				...
				if(!$homePage['success']) {
					...
					goaway('/gitwiki/'.$nick.'/'.$wiki['urlName']);
				}
				goaway('/gitwiki/'.$nick.'/'.$wiki['urlName'].'/'.$homePage['page']['urlName']);
			} else {
		...
		if ((argc() === 5) && (argv(2) === 'get') && (argv(3) === 'page') && (argv(4) === 'list')) {
			...
			$page_list_html = widget_gitwiki_pages(array(
					'resource_id' => $resource_id, 
					'refresh' => true, 
					'channel' => argv(1)));
		...
		if ((argc() === 4) && (argv(2) === 'history') && (argv(3) === 'page')) {
			...
			$historyHTML = widget_gitwiki_page_history(array(
				'resource_id' => $resource_id,
