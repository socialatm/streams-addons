...
function wiki_init_wiki($channel, $wiki) {
	...
	if (!os_mkdir(PROJECT_BASE . '/' . $path, 0770, true)) {
		logger('Error creating wiki path: ' . $path);
		return null;
	}
	// Create GitRepo object 	
	$git = new GitRepo($channel['channel_address'], null, false, $wiki['urlName'], PROJECT_BASE . '/' . $path);	
	if(!$git->initRepo()) {
...
function wiki_create_wiki($channel, $observer_hash, $wiki, $acl) {
	...
	$wiki_url = z_root() . '/gitwiki/' . $channel['channel_address'] . '/' . $wiki['urlName'];
...
function wiki_get_wiki($resource_id) {
	...
	if (!$item) {
		...
		if (! realpath(PROJECT_BASE . '/' . $path)) {
			return array('wiki' => null, 'path' => null);
		}
		// Path to wiki exists
		$abs_path = realpath(PROJECT_BASE . '/' . $path);
		return array(
			'wiki' => $w,
			'path' => $abs_path,
			'rawName' => $rawName,
