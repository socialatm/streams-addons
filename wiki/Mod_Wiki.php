...
require_once('addon/wiki/NativeWiki.php');
require_once('addon/wiki/NativeWikiPage.php');
...
class Wiki extends \Zotlabs\Web\Controller {
	...
	function get() {
		...
		$desc = t('This addon app provides a wiki for your channel.'); 

		$text = '<div class="section-content-info-wrapper">' . $desc . '</div>';

		if(! (\App::$profile_uid && Apps::addon_app_installed(\App::$profile_uid,'wiki'))) { 
			return $text;
		}
		...
		switch(argc()) {
			...
			default:
				...
				$langPageUrlName = \App::$language . '/' . $page_name;
				...
				if(! ($p && $p['success'])) {
							require_once('addon/wiki/Wiki_pages.php');
				    ...
				    $html = \NativeWikiPage::convert_links($html, argv(0) . '/' . argv(1) . '/' . \NativeWiki::name_encode($wikiUrlName));
					$renderedContent = \NativeWikiPage::generate_toc(zidify_text(MarkdownExtra::defaultTransform(\NativeWikiPage::bbcode($content))));
...
//				goaway('/' . argv(0) . '/' . argv(1) . '/' . $wikiUrlName . '/' . \NativeWiki::name_encode($pageUrlName));
		}
