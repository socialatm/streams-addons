...
function as_follow($channel,$act) {
	...
	if($contact && $contact['abook_id']) {
		...
		switch($act->type) {
			...
			case 'Accept':
				...
				set_abconfig($channel['channel_id'],$contact['abook_xchan'],'system','their_perms',implode(',',$their_perms));
	...
	if($their_perms) {
		set_abconfig($channel['channel_id'],$ret['xchan_hash'],'system','their_perms',implode(',',$their_perms));
	}
	...
	if($r) {
		...
		if($new_connection) {
			...
			if($my_perms && $automatic) {
				set_abconfig($channel['channel_id'],$ret['xchan_hash'],'system','my_perms',implode(',',$my_perms));
...
function as_unfollow($channel,$act) {
	...
	if(is_array($person_obj)) {
		...
		if($r) {
			// remove any permissions they provided
			del_abconfig($channel['channel_id'],$r[0]['xchan_hash'],'system','my_perms');
		}
	}
